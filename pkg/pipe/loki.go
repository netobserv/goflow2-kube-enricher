...
var (
	...
	llog        = logrus.WithField("module", "export/loki")
)
...
func (l *Loki) extractTimestamp(record map[string]interface{}) time.Time {
	...
	if !ok {
		llog.WithField("timestampLabel", l.config.TimestampLabel).
			Warnf("Timestamp label not found in record. Using local time")
		return l.timeNow()
	}
	ft, ok := getFloat64(timestamp)
	if !ok {
		llog.WithField(string(l.config.TimestampLabel), timestamp).
...
func (l *Loki) addNonStaticLabels(record map[string]interface{}, labels model.LabelSet) {
	...
	for _, label := range l.config.Labels {
		...
		if !sanitizedKey.IsValid() {
			llog.WithFields(logrus.Fields{"key": label, "sanitizedKey": sanitizedKey}).
				Debug("Invalid label. Ignoring it")
			continue
		}
		lv := model.LabelValue(fmt.Sprint(val))
		if !lv.IsValid() {
			llog.WithFields(logrus.Fields{"key": label, "sanitizedKey": sanitizedKey, "value": val}).
...
func getFloat64(timestamp interface{}) (ft float64, ok bool) {
	...
	case int:
		return float64(i), true
	default:
		llog.Warnf("Type %T is not implemented for float64 conversion\n", i)

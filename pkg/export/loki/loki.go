// Package loki enables data exporting to Loki
package loki
...
// Emitter abstracts the Loki Client
...
func buildLokiConfig(c *config.LokiConfig) (*loki.Config, error) {
	if err := c.Validate(); err != nil {
		return nil, fmt.Errorf("the provided loki config is not valid: %w", err)
	}
...
func (l *Loki) ProcessRecord(timestamp time.Time, record map[string]interface{}) error {
	...
	log.Debugf("Loki export labels %v timestamp %s", labels, timestamp.String())
